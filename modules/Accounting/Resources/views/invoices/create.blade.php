...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <label class="page-title">Nueva Factura</label>
        </section>
        ...
        <div class="box-body">
            <div class="form-group col-md-6 required {{ $errors->has('customer_id') ? 'has-error' : ''}}">
                ...
                <div class="input-group">
                    ...
                    {!! Form::select('customer_id', $customers, null, array_merge(['class' => 'form-control', 'placeholder' => 'Selecciona un cliente'])) !!}
            ...
            <div class="form-group col-md-6 required">
                <label for="date" class="control-label">Fecha de Emisión</label>
                <div class="input-group">
                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                    <input class="form-control" placeholder="Enter Date" id="date" required="required" data-inputmask="'alias': 'yyyy/mm/dd'" data-mask="" name="date" type="text" autocomplete="off" value={{date('d-m-Y')}}>
                </div>
            </div>

            <div class="form-group col-md-6 required ">
                <label for="due_date" class="control-label">Vencimiento</label>
                <div class="input-group">
                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                    <input class="form-control" placeholder="Enter Due Date" id="due_date" required="required" data-inputmask="'alias': 'yyyy/mm/dd'" data-mask="" name="due_date" type="text" autocomplete="off" value={{date('d-m-Y', strtotime("+1 month", strtotime(date('d-m-Y'))))}}>
                </div>
            </div>

            <div class="form-group col-md-6 required ">
                <label for="invoice_number" class="control-label">Nº Pedido</label>
                <div class="input-group">
                    <div class="input-group-addon"><i class="fa fa-shopping-cart"></i></div>
                    <input class="form-control" placeholder="Número de Pedido" required="required" name="order_number" type="text" value="" id="order_number" autocomplete="off">
                </div>
            </div>

            <div class="form-group col-md-6 required ">
                <label for="invoice_number" class="control-label">Nº Factura</label>
                <div class="input-group">
                    <div class="input-group-addon"><i class="fa fa-file-text-o"></i></div>
                    <input class="form-control" placeholder="Número de Factura" required="required" name="invoice_number" type="text" value="" id="invoice_number" autocomplete="off">
                </div>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" id="items">
                        ...
                        <tr id="item-row-{{ $item_row }}">
                            ...
                            <td>
                                {!! Form::select('item[' . $item_row . '][tax_id]', $taxes, 1, ['id'=> 'item-tax-'. $item_row, 'class' => 'form-control tax-select2', 'placeholder' => trans('general.form.select.field', ['field' => 'IVA'])]) !!}
                            </td>
                        ...
                        <tr>
                            <td class="text-right" style="vertical-align: middle;" colspan="5">
                                <a href="javascript:void(0)" id="discount-text" rel="popover">Añadir Descuento</a>
                            </td>
                            <td class="text-right">
...
@push('css')
<link rel="stylesheet" href="{{env('APP_URL')}}/vendor/select2/select2.min.css">
<link rel="stylesheet" href="{{env('APP_URL')}}/vendor/daterangepicker/daterangepicker.css">
<link rel="stylesheet" href="{{env('APP_URL')}}/vendor/datepicker/datepicker3.css">
@endpush

@push('js')
<script src="{{env('APP_URL')}}/vendor/select2/select2.min.js"></script>
<script src="{{env('APP_URL')}}/vendor/daterangepicker/moment.min.js"></script>
<script src="{{env('APP_URL')}}/vendor/daterangepicker/daterangepicker.js"></script>
<script src="{{env('APP_URL')}}/vendor/datepicker/bootstrap-datepicker.js"></script>
<script src="{{env('APP_URL')}}/vendor/datepicker/locales/bootstrap-datepicker.es.js"></script>
@endpush
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        ...
        $('a[rel=popover]').popover({
            ...
            content: function () {
                ...
                html += '            {!! Form::number('pre-discount', null, ['id' => 'pre-discount', 'class' => 'form-control text-right', 'style' => 'width: 70px']) !!}';
                ...
                html += '            {!! Form::button('<span class="fa fa-save"></span> &nbsp;' . 'Okay', ['type' => 'button', 'id' => 'save-discount','class' => 'btn btn-success']) !!}';
                html += '            <a href="javascript:void(0)" id="cancel-discount" class="btn btn-default"><span class="fa fa-times-circle"></span> &nbsp;Cancelar</a>';
                html += '       </div>';
                html += '    </div>';
                html += '</div>';
    ...
    function createCustomer() {
        ...
        @foreach($currencies as $currency_code => $currency_name)
                modal += '                              <option value="{{ $currency_code }}" {{ (1 == $currency_code) ? 'selected' : '' }}>{{ $currency_name }}</option>';
        @endforeach
                modal += '                          </select>';
        modal += '                      </div>';
        modal += '                  </div>';
